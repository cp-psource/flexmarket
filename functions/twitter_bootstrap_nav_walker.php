 ...
 class twitter_bootstrap_nav_walker extends Walker_Nav_Menu {
	...
	function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if (strcasecmp($item->title, 'divider')) {
			...
			$attributes .= ! empty($args->has_children) ? ' data-toggle="dropdown" data-target="#'. esc_attr( $item->url ) .'" class="dropdown-toggle"' : '';
			...
			$item_output .= ! empty($args->has_children) ? ' <span class="caret"></span></a>' : '</a>';
